const _urllib: any  = require("urllib")
const util = require('./wxPayV2Util');
...
class PaymentV2 {
  ...
  constructor({appid, mchid, partnerKey, pfx, notify_url, refund_url, spbill_create_ip, sandbox}: any = {}, debug = false) {
  ...
  static init(...args) {
    return new PaymentV2(...args);
  }

  static async sandbox(config, debug) {
    let {sandbox_signkey} = await PaymentV2.init(config).getSignkey();
  ...
  async _request(params, type, cert = false) {
    ...
    this.log('post data =>\r\n%s\r\n', pkg.data);
    let {status, data} = await _urllib.request(this.urls[type], pkg);
