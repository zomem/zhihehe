...
function FSaleList() {
  ...
  useEffect(() => {
    ...
    fetchPost('/management/manage/fsale/list', {
      page: page,
      limit: size,
    }, dispatch).then(res => {
      setInfo({
  ...
  // 通过selectId获取当前销总售员下面的所有销售
  useEffect(() => {
    if(selectId){
      fetchGet('/management/manage/fsale/saler/' + selectId, dispatch).then(res => {
        setUserinfo({
          list: res.data.list,
          loading: false,
          total: res.data.total
  ...
  return(
    <div>
      <div className='paths_top_con frsc'>
        <div style={{marginLeft: '25px'}}>共有{info.total}个总销售</div>
      </div>
      <div>
        <Table
      ...
      <Modal 
        title={`总销售${selectName}旗下的销售，共${userinfo.total}位`}
        visible={showModal} 
        onOk={() => {
          setShowModal(false)
          setSelectId(0)
